...
bool waitready (uint16_t timeout)
{
  ...
  if (timeout == 0) delay = portMAX_DELAY;
	...
	ESP_LOGE (TAG, "Waiting for IRQ");
  xQueueReceive(IRQQueue, &io_num, delay);
	ESP_LOGE (TAG, "IRQ Recieved");
...
{
  //TEST
  ESP_LOGI(TAG, "Initializing getPN532FirmwareVersion function");
  ...
  //TEST
  ESP_LOGI(TAG, "Sending command to get firmware version");

  if (!sendCommandCheckAck (pn532_packetbuffer, 1, I2C_WRITE_TIMEOUT))
  {
    //TEST
     ESP_LOGE(TAG, "Failed to get acknowledgment for firmware version command");
    return 0;
  }
  
  //TEST
  ESP_LOGI(TAG, "Command acknowledged, reading data packet");

  // read data packet
  readdata (pn532_packetbuffer, 12);

  //TEST
  ESP_LOGI(TAG, "Data packet read. Data: ");
    for (int i = 0; i < 12; i++) {
        ESP_LOGI(TAG, "Data[%d]: 0x%02X", i, pn532_packetbuffer[i]);
    }
...
bool SAMConfig (uint8_t irq, uint8_t timeout_multiplier)
{
  ...
  pn532_packetbuffer[3] = irq; // use IRQ pin!
...
#ifdef CONFIG_PN532DEBUG
  ESP_LOGI(TAG, "Waiting for IRQ (indicates card presence)");
#endif
  if (!waitready (timeout))
  {
#ifdef CONFIG_PN532DEBUG
    ESP_LOGI(TAG, "IRQ Timeout");
#endif
    return 0x0;
  }
